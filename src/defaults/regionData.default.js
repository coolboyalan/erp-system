...
(async function addRegion() {
  session.run(async () => {
    try {
      ...
      for (const state in region) {
        const createdState = await State.create({ name: state, countryId });
        const stateId = createdState.id;

        for (let city of region[state]) {
          const createdCity = await City.create({ name: city, stateId });
        }
      }
